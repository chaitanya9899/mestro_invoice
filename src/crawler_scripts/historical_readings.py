...
def HistoricalReadings(agentRunContext):
    ...
    try:
        ...
        for div in divs:
            ...
            # important_ds.append({useplace_id:{utility_id:service_identifier}})
            if important_ds.get(useplace_id) is not None:
                this_iteration_dict = {utility_id:service_identifier}
                if this_iteration_dict not in important_ds[useplace_id]:
                    important_ds[useplace_id].append(this_iteration_dict)
            else:
        ...
        for i in range(len(useplace_id_chronological)):
            ...
            utility_type_container_select = Select(driver.find_element_by_id('UtilityDropDownMenu'))

            total_options = len(utility_type_container_select.options) - 1
            ...
            for j in range(total_options):
                ...
                utility_type_container_button.click()
                time.sleep(2)
                dropdown_menu_utility = utility_type_container.find_element_by_class_name('dropdown-menu')
                dropdown_menu_ul_utility = dropdown_menu_utility.find_element_by_tag_name('ul')
                curr_li_utility = dropdown_menu_ul_utility.find_elements_by_tag_name('li')[j]
                address_click_a_utility = curr_li_utility.find_element_by_tag_name('a')
                address_click_a_utility.click()
                wait.until(EC.invisibility_of_element_located((By.CLASS_NAME,"ajax-loading")))
                time.sleep(3)
                ...
                if important_ds.get(useplace_id_chronological[i]) is not None:
                    ...
                    facilityId_dict = important_ds.get(useplace_id_chronological[i])[j]
                    print(facilityId_dict,utility_type,useplace_id_chronological[i])
                    facilityId = facilityId_dict[access_key]
                    print(facilityId)
